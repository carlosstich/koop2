...
function createClause (options) {
  ...
  var whereTransform = traverse(whereTree, options)
...
function translateObjectIdFragments (input, options) {
  ...
  var metadataObjectIdField = _.get(options, 'collection.metadata.fields', []).find(function (field) { return field.name === 'OBJECTID' })
  ...
  if (where.includes('OBJECTID') && !metadataObjectIdField && !idField) {
    ...
    var regexOid1st = /(properties|attributes)->`OBJECTID` (=|<|>|<=|>=) ([0-9]+)/g

    // RegExp for value-first predicate, e.g "1234 = properties->`OBJECTID`""
    var regexOid2nd = /([0-9]+) (=|<|>|<=|>=) (properties|attributes)->`OBJECTID`/g
