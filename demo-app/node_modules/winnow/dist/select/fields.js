 ...
 */
function createClause (options, idField) {
  if ( options === void 0 ) options = {};
  if ( idField === void 0 ) idField = null;

  // Default clause
  var clause = "type, properties as properties"

  // Comma-delimited list of date-fields is needed for formatting ESRI specific output
  var dateFields = options.dateFields.join(',')
  var requiresObjectId = !!options.returnIdsOnly || !(options.fields instanceof Array && !options.fields.includes('OBJECTID'))
  ...
  if (options.fields) {
    ...
    var fields = (options.fields instanceof Array) ? options.fields.join(',') : options.fields.replace(/,\s+/g, ',')
