...
function getFieldValues (features, field) {
  ...
  var values = features.map(function (feature, index) {
    var properties = feature.properties
    var key = Object.keys(properties).filter(function (property) { return property === field })
...
function normalizeByField (values, features, classification) {
  if (classification.normField) {
    var normValues = getFieldValues(features, classification.normField)
...
function normalizeByPercent (values) {
  var valueTotal = values.reduce(function (sum, value) { return sum + value }, 0)
  if (valueTotal <= 0) { throw new Error('Value total is not positive: ' + valueTotal) }
  return values.map(function (value) { return (value / valueTotal) * 100 })
}

module.exports = { getFieldValues: getFieldValues, normalizeClassBreaks: normalizeClassBreaks }
