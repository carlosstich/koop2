...
function normalizeGeometry (options) {
  ...
  var geometryFilterSR = bboxCRS || inSR
  var sourceSR = normalizeSourceSR(options.sourceSR)
...
function normalizeSourceSR (input) {
  var spatialReference = normalizeSR(input)
  if (spatialReference) { return ((spatialReference.wkid) ? ("EPSG:" + (spatialReference.wkid)) : spatialReference.wkt) }
...
function normalizeSR (input) {
  ...
  var knownWkids = [4326, 4269, 3857, 3785, 900913, 102113]
  var inputWkid
  ...
  var wkt = wkidLookup[inputWkid]
  if (wkt) { return { wkt: wkt } }
...
function normalizeLimit (options) {
  var limit = options.limit || options.resultRecordCount || options.count || options.maxFeatures
