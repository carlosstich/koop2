...
function query (data, params) {
  ...
  var filtersApplied = data.filtersApplied || {}
  var options = _.cloneDeep(params)
  var hasIdField = _.has(data, 'metadata.idField')
  ...
  var queriedData = filtersApplied.all ? data : Winnow.query(data, options)
...
function geoservicesPostQuery (data, queriedData, params) {
  var oidField = _.get(data, 'metadata.idField') || 'OBJECTID'
  ...
  if (params.objectIds && !params.outStatistics) {
    ...
    if (Array.isArray(params.objectIds)) { oids = params.objectIds }
      ...
      var error = new Error("Invalid \"objectIds\" parameter.")
      error.code = 400
      throw error
    }
...
function idsOnly (data) {
  var oidField = _.get(data, 'metadata.idField') || 'OBJECTID'
  var response = { objectIdFieldName: oidField, objectIds: [] }
  response.objectIds = data.features.map(function (f) { return f.attributes[oidField] })
  return response
}
...
function warnOnMetadataFieldDiscrepencies (metadataFields, featureProperties) {
  ...
  var featureFields = Object.keys(featureProperties).map(function (name) {
    return { name: name, type: Utils.detectType(featureProperties[name]) }
  })
  ...
  metadataFields.filter(function (field) {
    // look for a defined field in the features properties
    var featureField = _.find(featureFields, ['name', field.name]) || _.find(featureFields, ['name', field.alias])
  ...
  featureFields.filter(function (field) {
    var noNameMatch = _.find(metadataFields, ['name', field.name])
    var noAliasMatch = _.find(metadataFields, ['alias', field.name])
...
function getExtent (geojson, outSR) {
  ...
  var wkid = outSR.latestWkid || outSR.wkid || outSR
